...
graph TD
    %% Khởi động hệ thống
    Start((Khởi động mạch ESP32)) --> Setup[Khởi tạo GPIO, Đọc Cấu Hình Preferences]
    Setup --> WifiInit[Phát WiFi AP 'SmartSac' & Kết nối Station WiFi]
    WifiInit --> TaskInit[Khởi động Web Server, Telegram Bot, Cloud Sync]
    TaskInit --> MainLoop
    
    %% Vòng lặp chính
    subgraph VÒNG LẶP CHÍNH (MAIN LOOP)
        MainLoop{Bắt đầu chu kỳ}
        
        %% 1. Xử lý cảm biến & Auto-disable
        MainLoop --> ReadSensors[Đọc cảm biến: DS18B20, MLX, DHT, PZEM]
        ReadSensors --> CheckSensorError{Có cảm biến nào bị hỏng/mất tín hiệu?}
        CheckSensorError -- Có --> AutoDisable[Auto-Disable: Tắt cờ feat_*, Lưu Preferences, Trả về N/A]
        AutoDisable --> CheckLocalSafety
        CheckSensorError -- Không --> CheckLocalSafety
        
        %% 2. Xử lý an toàn nội bộ
        CheckLocalSafety{Vượt ngưỡng an toàn nội bộ?\n(Nhiệt độ, Độ ẩm, P_max)}
        CheckLocalSafety -- Có --> LocalAlarm[Ngắt Relay, Đặt trạng thái LOI]
        LocalAlarm --> DisplayError[LCD/Telegram hiện thông báo lỗi]
        CheckLocalSafety -- Không --> CloudSync
        
        %% 3. Xử lý Cloud & An toàn từ xa
        CloudSync[Gửi Telemetry lên Cloud Server] --> CheckCloudError{Server trả về mã HTTP 400?}
        CheckCloudError -- Có --> CloudLockout[Safety Lockout: Ngắt Relay, Khóa hệ thống]
        CloudLockout --> FilterAccent[Lọc dấu tiếng Việt chuỗi JSON lỗi]
        FilterAccent --> DisplayCloudError[In đè cảnh báo an toàn lên toàn màn hình LCD]
        
        %% 4. Xử lý Trạng Thái Sạc
        CheckCloudError -- Không --> ChargeStateMachine{Trạng thái (State) hiện tại?}
        
        ChargeStateMachine -- "SAC_CHO / DANG_HEN_GIO" --> WaitTimer[Đếm ngược thời gian chờ T_cho]
        WaitTimer --> CheckWait{Hết thời gian chờ?}
        CheckWait -- Có --> TriggerCharge[Bật Relay, Chuyển sang DANG_SAC]
        CheckWait -- Không --> UIUpdate
        
        ChargeStateMachine -- "DANG_SAC" --> CheckFull{PZEM: Công suất < P_day trong T_xacnhan giây?}
        CheckFull -- Có --> StateFull[Ngắt Relay, Chuyển sang FULL]
        StateFull --> ShowKwh[LCD Dòng 2: Hiển thị số kWh]
        ShowKwh --> UIUpdate
        
        CheckFull -- Không --> CheckMaxTime{Vượt quá thời gian sạc tối đa (gio_max)?}
        CheckMaxTime -- Có --> MaxTimeStop[Ngắt Relay, Chuyển sang DUNG]
        CheckMaxTime -- Không --> KeepCharging[Duy trì Relay BẬT]
        MaxTimeStop --> UIUpdate
        KeepCharging --> UIUpdate
        
        ChargeStateMachine -- "DUNG / FULL / LOI" --> UIUpdate
        
        %% 5. Giao tiếp & UI
        UIUpdate[Cập nhật màn hình LCD Normal Mode & Phản hồi Telegram Bot] --> APCheck
        
        %% 6. AP Timeout
        APCheck{Chế độ AP_Timeout?\n(feat_ap_on=true & ap_always=false)}
        APCheck -- Đã qua 5 phút --> DisableAP[Tắt phát sóng WiFi AP nội bộ]
        APCheck -- Chưa qua 5 phút / Bỏ qua --> InputCheck
        DisableAP --> InputCheck
        
        %% 7. Chờ lệnh điều khiển
        InputCheck[Nhận lệnh: Nút nhấn BTN3, Telegram Bot, Web Portal] --> MainLoop
    end
    
    %% Đường dẫn từ lỗi về UI
    DisplayError -.-> InputCheck
    DisplayCloudError -.-> InputCheck
    TriggerCharge -.-> UIUpdate
